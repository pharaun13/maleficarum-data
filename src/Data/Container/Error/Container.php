...
trait Container {
    ...
    public function mergeErrors(array $errors) {
        if (!array_key_exists('__default_error_namespace__', $errors)) {
            throw new \InvalidArgumentException(sprintf('Incorrect error set provided. \%s::mergeErrors()', static::class));
        }
        ...
        foreach ($errors as $namespace => $errs) {
            ...
            foreach ($errs as $err) {
                ...
                if (!array_key_exists('code', $err) || !preg_match('/^\d{4}\-\d{6}$/', $err['code'])) {
                    continue;
                }
                if (!array_key_exists('msg', $err) || !is_string($err['msg'])) {
                    continue;
                }
    ...
    public function setErrors(array $errors) {
        if (!array_key_exists('__default_error_namespace__', $errors)) {
            throw new \InvalidArgumentException(sprintf('Incorrect error set provided. \%s::setErrors()', get_class($this)));
        }
    ...
    public function addError(string $code, string $message, string $namespace = '__default_error_namespace__', bool $duplicateToDefault = true) {
        if (!preg_match('/^\d{4}\-\d{6}$/', $code)) {
            throw new \InvalidArgumentException(sprintf('Incorrect error code format. \%s::addError()', static::class));
        }
